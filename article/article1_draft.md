...
Out at the railroad, each of the seven stations has a small fascia-mounted touchscreen unit — an ESP32 "Cheap Yellow Display," CYD for short — sitting where a station agent's desk would have been in 1905. It shows the fast clock, takes OS (on-sheet) reports as trains arrive and depart, and displays train orders and clearance forms as the dispatcher issues them. Five of the seven stations, between the terminal stations, also drive a small servo-actuated signal arm — dropped to horizontal, the stop position, the instant the dispatcher issues an order addressed to that station, and raised back to vertical once the order is acknowledged. All NY&E orders are Form 31: the crew must physically stop at the station and sign for the order before proceeding. The arm is the visible cue for that moment, exactly as a train order signal worked on the prototype.
...
The best way to see how these roles fit together is to watch one real meet happen, start to finish — pulled straight from the published Timetable No. 4, not a hypothetical. *(The full employee timetable and string table for Timetable No. 4 are published at the layout's operations page, modelrr.adbyrne.us/operation.html.)* No. 52 South is the daily local way freight, second class, scheduled to leave Hemlock Crest at 6:15 AM and work every station on its way down to the yard. At the same moment, an extra freight — with Engine 14, identified as X 14 North — gets TO Form G - "Running Orders" - to leave Williamsport northbound. The two will likely converge on Jacks Creek, the branch's main passing siding — but getting them through in the right order takes a train order, not just the timetable, because of how the two trains rank against each other.

This is worth pausing on, because it's a rule a lot of newer operators get backwards: direction only decides superiority between trains of the *same* class. Class comes first. An extra has no rights of its own at all — it's inferior to every scheduled train on the timetable, schedule or no schedule, regardless of which way either one is headed. Left to the default rules, No. 52 South has every right to hold the main at Jacks Creek for its scheduled 10-minute work stop and highball south right on time at 7:09, extra or no extra. The dispatcher wants Extra 14 North kept moving, so Extra 14 North gets a second order at BB — a Form A meet, the Standard Code form that fixes a meeting point without specifying which train takes the siding; the rules settle that. No. 52 South gets the same order at MC. The dispatcher app tracks both: which of its structured order types supplies the fields (a meet, in this case), and which of the rulebook's lettered forms the order actually represents, since that's what determines what authority it's carrying.
...
**[Figure 6 — Station CYD touchscreen, four states: `docs/diagrams/cyd_screens/`]**

*(1) Clock:* Default screen — station name, railroad time, next trains in each direction. Tap to start an OS report.
*(2) OS Entry:* Crew keys in the train number and direction (N/S/Extra/Work Extra) on the built-in keypad.
*(3) Train Orders:* Shown automatically if a train order is waiting for this train. ACK confirms receipt back to the dispatcher.
*(4) Next Station:* Confirms the OS report and shows the next station up the line, plus the next opposing train due there.

> 6:15 AM — No. 52 South reports at HC (departing)
> 6:15 AM — Extra 14 North reports at WP (departing)
> 6:24 AM — No. 52 South reports at SK
> 6:33 AM — Extra 14 North reports at XP
> 6:48 AM — Extra 14 North reports at BB
> 6:51 AM — No. 52 South reports at MC
> 7:09 AM — No. 52 South reports at JC

No. 52 South reports in at Michelles Cove right on schedule, set to do its usual ten minutes of work and head out again at 7:01 — but Extra 14 North is already past Becs Bend and closing fast. The dispatcher had already issued a Form A order, addressed to Michelles Cove and Becs Bend, scheduling the meet at Jacks Creek:

> No. 52 will meet Extra 14 North at Jacks Creek.
...
- [x] Byline / AI co-authorship disclosure — resolved 2026-06-25: editor Eric Smith does not want Claude in the byline; byline is now `By Allen D. Byrne` only. AI contribution woven into Section 6 narrative as intended.
...
- [x] **Form A / Form 31 / signal arm corrections, 2026-06-24.** Changed meet order from Form C ("take siding and wait for") to Standard Code Form A ("will meet … at") — per Forman (1904), Form A fixes the meeting point without explicitly assigning the siding; timetable rights settle that. Order text updated to `No. 52 will meet Extra 14 North at Jacks Creek.` — direction suffix removed from No. 52 (standard: scheduled trains identified by number only; extras by engine + direction). Section 3 signal arm corrected: arm *drops* to horizontal (stop) when order issued, *raises* back to vertical when acknowledged — matching C&O horizontal=stop convention. Form 31 (crew must stop and sign) added to Section 3. OS log corrected: "No. 52 leaves BB" → "leaves JC" (southbound: JC is first stop after MC, not BB). Figure 6 captions added from `controlsystem/screens.html`. `to_types.json` meet type updated to Form A template; `default_form` changed from "19" to "31". CYD screen renders regenerated via new `docs/scripts/gen_cyd_screens.py`. Note: the prior item below recording Form C order text is superseded by this pass.
